...
const YearsFilter: React.FC<YearsFilterProps> = ({
  ...
  const flatYears = years.flat();
  const filteredYears = flatYears.filter((year) =>
    year.toString().includes(searchTerm)
  );

  const handleSelect = (year: string) => {
    const newValue = value?.includes(year) ? [] : [year];
    onChange(newValue);
  };
    ...
    <div className="absolute top-full left-0 mt-2 w-96 bg-white rounded-lg shadow-xl z-50 p-6 max-h-96 overflow-y-auto">
      <div className="space-y-4">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-lg font-semibold text-gray-900">Years</h3>
          <button
            onClick={handleClear}
