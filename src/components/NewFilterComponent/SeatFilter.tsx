...
const SeatsFilter: React.FC<SeatsFilterProps> = ({
  value = [],
  onChange,
  onClose,
  compact = false,
  ...
  const handleSelect = (seatValue: string) => {
    const newValue = value?.includes(seatValue) ? [] : [seatValue];
    onChange(newValue);
  };
    ...
    <div className="absolute top-full left-0 mt-2 w-96 bg-white rounded-lg shadow-xl z-50 p-6 max-h-96 overflow-y-auto">
      <div className="space-y-4">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-lg font-semibold text-gray-900">Seats</h3>
          <button
            onClick={handleClear}
